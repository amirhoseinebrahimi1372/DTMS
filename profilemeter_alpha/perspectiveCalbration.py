...
class cameraGraber:
    def __init__(self, idx):
        ...
        self.camera.GainRaw.SetValue(60)
        ...
        self.camera.ExposureTimeAbs.SetValue(500)
...
class calibration_perspective():
    ...
    def __init__(self, path, camera_calibration, chess_size = (9,6), chess_home_size = 30, border = 2 ):
        ...
        self.camera_calibration.load()


    def center_calibration(self, cameras, debug=True):
            ...
            while True:
                ...
                if debug:
                    ...
                    while True:
                        img = next( cam.grabber())
                        dst = cv2.warpPerspective( img, M, (dst_w, dst_h))
                        cv2.circle(dst, (int(dst.shape[1]/2),  int(dst.shape[0]/2), ), 3, (0,0,255), thickness=-1)
                        cv2.imshow('image', cv2.resize(img, None, fx=0.75, fy=0.75 ))
                        cv2.imshow('dst ' + cam_pos, cv2.resize(dst, None, fx=0.75, fy=0.75 ))
                        key = cv2.waitKey(50)
                        t+=1
                        if key == ord('n') or key == 27 or  key == ord('y')or t>1000:
                            cv2.destroyAllWindows()
                            break
                    if key == 27 or key == ord('y') or t>20:
                        break
...
if __name__ == '__main__':
    ...
    perspective = calibration_perspective('data/perspective_calib', camera_calibration, chess_size = (6,6), chess_home_size = 20, border = 3)
